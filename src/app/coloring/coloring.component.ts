...
export class ColoringComponent implements OnInit, OnDestroy {
  ...
  public constructor(
    ...
    this._queryObservable = route.params.pipe(
      map((params: Params) => {
        ...
        return Number.isNaN(param) ? null : Number(param);
      })
    );
  }
  ...
  private _fileLoad(files: FileList): void {
    ...
    const clear =
      this._coloringSettings.settings.fileName !== fileToUpload.name;
    this._coloringSettings.settings = {
      ...this._coloringSettings.settings,
  ...
  private _load(clear: boolean, fileToUpload?: File): void {
    ...
    (fromCash
      ? of(fromCash)
      : fileToUpload
      ? this._http.convertToPoints<IRecColor>(
          fileToUpload,
          this._coloringSettings.settings
        )
      ...
      .subscribe((res: IRecColor) => {
        ...
        this._coloringHelper.onUpdateCanvas.emit({
          recColor: res,
          clear: clear,
